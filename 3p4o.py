...
paramMins = np.array([0, 0.5, 0.5])  # [param1_min, param2_min, ..., paramN_min]
paramMaxs = np.array([0.5, 1.2, 4.0])  # [param1_max, param2_max, ..., paramN_max]
...
def trentoRun(params, nTrent, uncert=False):
    ...
    with subprocess.Popen(string.split(), stdout=subprocess.PIPE) as proc:
        data4 = np.array([l.split() for l in proc.stdout], dtype=float)[:, 7]
    aveg = np.mean(data)
    aveg2 = np.mean(data2)
    aveg3 = np.mean(data3)
    aveg4 = np.mean(data4)
