...
def trentoRun(params, nTrent, uncert=False):
    ...
    if uncert:
        ...
        return np.array([(aveg, aveg2, aveg3), (std1, std2, std3)])
    return np.array([(aveg, aveg2, aveg3)])
...
obsLabels = np.array([r"$\epsilon$2", r"$\epsilon$3", r"$\epsilon$4"])
...
def saving(aa):
    ...
    accessFileName = "./e4/" + str(totDesPts) + "dp" + str(nTrentoRuns) + "tr"
    dpFileName = "./e4/" + str(totDesPts) + "dp" + str(nTrentoRuns) + "trDataDP"
    obsFileName = "./e4/" + str(totDesPts) + "dp" + str(nTrentoRuns) + "trDataObs"
