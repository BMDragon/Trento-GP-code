...
def trentoRun(params, nTrent, uncert=False):
    ...
    with subprocess.Popen(string.split(), stdout=subprocess.PIPE) as proc:
        data3 = np.array([l.split() for l in proc.stdout], dtype=float)[:, 6]
    with subprocess.Popen(string.split(), stdout=subprocess.PIPE) as proc:
        data4 = np.array([l.split() for l in proc.stdout], dtype=float)[:, 6]
    aveg = np.mean(data)
    aveg2 = np.mean(data2)
    aveg3 = np.mean(data3)
    aveg4 = np.mean(data4)
    if uncert:
        ...
        std3 = np.std(data3) / np.sqrt(nTrent)
        std4 = np.std(data4) / np.sqrt(nTrent)
        return np.array([(aveg, aveg2, aveg3, aveg4), (std1, std2, std3, std4)])
    return np.array([(aveg, aveg2, aveg3, aveg4)])
...
paramLabels = np.array(["Reduced thickness", "Nucleon-Width", "Fluctuation"])
paramMins = np.array([0, 0.5, 0.5])
paramMaxs = np.array([0.5, 1.2, 4.0])
obsLabels = np.array([r"$\epsilon$2", r"$\epsilon$3", r"$\epsilon$4", r"$\epsilon$5"])
paramTruths = np.array([0.314, 0.618, 1.234])
...
def saving(aa):
    ...
    accessFileName = "./e5/" + str(totDesPts) + "dp" + str(nTrentoRuns) + "tr"
    dpFileName = "./e5/" + str(totDesPts) + "dp" + str(nTrentoRuns) + "trDataDP"
    obsFileName = "./e5/" + str(totDesPts) + "dp" + str(nTrentoRuns) + "trDataObs"
