...
paramLabels = np.array(["Fluctuation", "Nucleon-Width"])
paramMins = np.array([0.5, 0.5])  # [param1_min, param2_min, ..., paramN_min]
paramMaxs = np.array([4.0, 1.2])  # [param1_max, param2_max, ..., paramN_max]
obsLabels = np.array([r"$\epsilon$2", r"$\epsilon$3"])
paramTruths = np.array([1.234, 0.618])
dataUncert = 65536


# Trento options
def trentoRun(params, nTrent, uncert=False):
    string = '../build/src/trento Pb Pb ' + str(nTrent) + ' -k ' + str(params[0]) + ' -w ' + str(params[1])
